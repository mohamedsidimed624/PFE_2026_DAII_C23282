...
const ELECTION_PRESETS = {
  CONSEIL_NATIONAL: {
    ...
    positions: [
      {
        libelle: "Section A — Médecins généralistes",
        nombreSieges: 10,
        maxVotesParElecteur: 10,
        ordre: 1,
      },
      {
        libelle: "Section B — Médecins spécialistes",
        nombreSieges: 6,
        maxVotesParElecteur: 6,
        ordre: 2,
      },
      {
        libelle: "Section C — Médecins enseignants-chercheurs",
        nombreSieges: 6,
        maxVotesParElecteur: 6,
        ordre: 3,
      },
      {
        libelle: "Représentants des régions de l'intérieur",
        nombreSieges: 3,
        maxVotesParElecteur: 3,
        ordre: 4,
      },
    ],
  },
  ...
  BUREAU_EXECUTIF: {
    ...
    positions: [
      ...
      {
        libelle: "Secrétaire général adjoint",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 4,
      },
  ...
  BUREAU_SECTION_A: {
    ...
    positions: [
      {
        libelle: "Président de la Section A",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 1,
      },
      {
        libelle: "Secrétaire général de la Section A",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 2,
      },
      {
        libelle: "Trésorier de la Section A",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 3,
      },
  ...
  BUREAU_SECTION_B: {
    ...
    positions: [
      {
        libelle: "Président de la Section B",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 1,
      },
      {
        libelle: "Secrétaire général de la Section B",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 2,
      },
      {
        libelle: "Trésorier de la Section B",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 3,
      },
  ...
  BUREAU_SECTION_C: {
    ...
    positions: [
      {
        libelle: "Président de la Section C",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 1,
      },
      {
        libelle: "Secrétaire général de la Section C",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 2,
      },
      {
        libelle: "Trésorier de la Section C",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 3,
      },
  ...
  REPRESENTANTS_REGIONAUX: {
    label: "Représentants des régions de l'intérieur",
    description:
    ...
    positions: [
      {
        libelle: "Représentant de la région",
        nombreSieges: 1,
        maxVotesParElecteur: 1,
        ordre: 1,
      },
...
const CORPS_LABELS = {
  ...
  MEDECINS_PAR_SECTION: "Médecins inscrits, répartis selon leur section",
      ...
      {STEPS.map((label, i) => (
        <div key={i} className="flex items-center flex-1 last:flex-none">
          <div className="flex flex-col items-center">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-full text-[11px] font-bold transition-all ${
                i < current
                  ...
                  : i === current
                  ? "border-2 border-emerald-600 bg-white text-emerald-600 dark:bg-slate-900"
            ...
            <span
              className={`mt-1 text-[10px] font-semibold whitespace-nowrap hidden sm:block ${
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`flex-1 h-px mx-2 -mt-3 ${
                i < current
                  ? "bg-emerald-500"
    ...
    <div className="flex items-start mt-5">
      {points.map((p, i) => (
        <div key={i} className="flex-1 flex flex-col items-center">
          ...
          <p className="mt-1.5 text-center text-[10px] font-semibold text-slate-400 leading-tight px-1">
            {p.label}
          </p>
      ...
      <div className="space-y-3">
        ...
        {form.titre && (
          <div>
            ...
            <p className="text-[12px] text-slate-600 dark:text-slate-300 wrap-break-word leading-snug">
              {form.titre}
            </p>
          </div>
        )}
                  ...
                  val && (
                    <div key={lbl} className="flex justify-between text-[10px]">
                      ...
                      <span className="font-medium text-slate-600 dark:text-slate-300">
                        {new Date(`${val}:00`).toLocaleDateString("fr-FR", {
                          day: "2-digit",
                          month: "short",
                        })}
                      </span>
          ...
          <div>
            ...
            <div className="space-y-1">
              {preset.positions.map((p, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-700/40 px-2.5 py-1.5"
                >
                  <span className="text-[10px] text-slate-600 dark:text-slate-300 truncate pr-2 leading-snug">
                    {p.libelle}
                  </span>
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-[12px] text-slate-500 mt-0.5">
          Choisissez l'organe concerné. Les postes et le corps électoral sont
      ...
      {locked && (
        <div className="rounded-xl border border-amber-100 bg-amber-50 dark:border-amber-900/30 dark:bg-amber-900/10 px-4 py-3 text-[12px] text-amber-700 dark:text-amber-400">
          Le type d'élection est verrouillé — les candidatures ont déjà été
          ouvertes.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {Object.entries(ELECTION_PRESETS).map(([key, p]) => {
          ...
          const isBlocked =
            p.type !== "REPRESENTANTS_REGIONAUX" && blockedTypes.has(p.type);
          const hasRegionalConflict =
            p.type === "REPRESENTANTS_REGIONAUX" && blockedTypes.has(p.type);
          const isDisabled = locked || isBlocked;
          ...
          return (
            <button
              ...
              type="button"
              disabled={isDisabled}
              onClick={() => !isDisabled && setPresetKey(selected ? "" : key)}
              className={`text-left rounded-xl border p-4 transition-all ${
                isBlocked
                  ? "border-red-200 bg-red-50/40 dark:border-red-900/40 dark:bg-red-900/5 opacity-70 cursor-not-allowed"
                  : selected
                  ? "border-2 border-emerald-500 bg-emerald-50/60 dark:bg-emerald-900/10 cursor-pointer"
                  : locked
                  ? "border-slate-200 dark:border-slate-700 opacity-60 cursor-not-allowed"
                  : "border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 hover:bg-slate-50 dark:hover:bg-slate-800/60 cursor-pointer"
              }`}
            >
              ...
              {isBlocked && (
                <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20 px-2.5 py-1.5 text-[10px] font-semibold text-red-600 dark:text-red-400">
                  Élection active en cours — veuillez l'archiver ou l'annuler avant de recréer.
                </div>
              )}

              {hasRegionalConflict && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20 px-2.5 py-1.5 text-[10px] font-semibold text-amber-700 dark:text-amber-400">
                  Certaines régions ont déjà une élection active — vérifiez à l'étape suivante.
                </div>
              )}
            </button>
          );
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[12px] text-slate-500 mt-0.5">
          Nommez l'élection et rédigez une description publique.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 px-4 py-3">
        <div className="flex flex-wrap gap-x-5 gap-y-1 text-[12px]">
          ...
          <span className="text-slate-500">
            Corps :{" "}
            <span className="font-semibold text-slate-700 dark:text-slate-200">
              {CORPS_LABELS[preset.corpsElectoral]}
            </span>
          </span>
      ...
      <Field
        ...
        required
        error={errors.titre}
        help="Entre 5 et 120 caractères."
      >
        <input
          type="text"
          maxLength={120}
      ...
      <Field
        ...
        help="Optionnel. Visible par les médecins sur leur espace."
      >
        <textarea
          rows={3}
        ...
        <Field label="Région concernée" required error={errors.region}>
          ...
          {!errors.region &&
            ...
            blockedRegions.has(form.region.toLowerCase()) && (
              <div className="mt-1.5 rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/10 px-3 py-2 text-[11px] font-semibold text-red-600 dark:text-red-400">
                Une élection active existe déjà pour cette région. Veuillez l'archiver ou l'annuler avant d'en créer une nouvelle.
              </div>
            )}
        </Field>
      )}
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[12px] text-slate-500 mt-0.5">
          Définissez les 4 dates clés du processus électoral.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {fields.map(({ key, label }) => (
          <Field key={key} label={label} required error={errors[key]}>
            <input
              type="datetime-local"
              min={key === "candidatureStartDate" ? nowLocal() : undefined}
              value={form[key]}
...
function StepApercu({ preset, form, onDraft, saving }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
        {rows.map(([lbl, val]) => (
          <div key={lbl} className="flex gap-4 px-4 py-2.5">
            ...
            <span className="text-[13px] text-slate-700 dark:text-slate-200 wrap-break-word">
              {val}
            </span>
          </div>
        ))}
      ...
      <div className="pt-1">
        ...
        <p className="mt-2 text-[11px] text-slate-400">
          Les candidatures pourront être ouvertes depuis la page détail lorsque la date prévue sera atteinte.
        </p>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 shadow-xl">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer select-none">
            ...
            <span className="text-[13px] text-slate-600 dark:text-slate-300">
              J'ai vérifié toutes les informations et je confirme l'enregistrement de cette élection.
            </span>
          </label>
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 dark:border-slate-800 px-6 py-4">
          ...
          <button
            ...
            onClick={onConfirm}
            className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-[13px] font-semibold text-white hover:bg-emerald-700 disabled:opacity-40"
          >
...
export default function AdminElectionCreationPage() {
  ...
  useEffect(() => {
    ...
    getElectionById(id)
      ...
      .catch(() => {
        setSaveError("Impossible de charger l'élection.");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  function validateStep(targetStep) {
    ...
    if (targetStep > STEP_TYPE) {
      if (!presetKey) {
        ...
        if (
          ...
          e.presetKey =
            "Une élection active de ce type existe déjà. Veuillez l'archiver ou l'annuler avant d'en créer une nouvelle.";
        }
      }
    }
    ...
    if (targetStep > STEP_INFO) {
      ...
      if (preset?.requiresRegion && !form.region) {
        e.region = "La région est obligatoire pour ce type d'élection.";
      } else if (
        preset?.requiresRegion &&
        form.region &&
        blockedRegions.has(form.region.toLowerCase())
    ...
    if (targetStep > STEP_CALENDRIER) {
      const {
        candidatureStartDate: cs,
        candidatureEndDate: ce,
        voteStartDate: vs,
        voteEndDate: ve,
      } = form;
      ...
      if (cs && ce && vs && ve) {
        // const now = new Date();
        // const candidatureStart = new Date(cs);
        // const candidatureEnd = new Date(ce);
        // const voteStart = new Date(vs);
        // const voteEnd = new Date(ve);

        // if (candidatureStart < now) {
        //   e.candidatureStartDate =
        //     "L'ouverture des candidatures ne peut pas être dans le passé.";
        // } else if (candidatureStart >= candidatureEnd) {
        //   e.calendar = "La clôture des candidatures doit être après l'ouverture.";
        // } else if (candidatureEnd >= voteStart) {
        //   e.calendar = "L'ouverture du vote doit être après la clôture des candidatures.";
        // } else if (voteStart >= voteEnd) {
  ...
  async function syncPositions(electionId) {
    ...
    for (const p of existing) {
      await deletePosition(electionId, p.id);
    }

    for (const p of preset.positions) {
      await addPosition(electionId, p);
    }
  }

  async function submit() {
    if (!validateStep(STEP_APERCU + 1)) {
      setShowModal(false);
      return;
    }
    ...
    if (isEdit && electionStatut !== "BROUILLON") {
      setSaveError(
        "Cette élection ne peut plus être modifiée car elle n'est plus en brouillon."
      );
      setShowModal(false);
      return;
    }
