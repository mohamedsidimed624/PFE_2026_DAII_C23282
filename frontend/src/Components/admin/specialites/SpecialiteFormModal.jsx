...
function SpecialiteFormModal({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      ordreAffichage:
        initialData?.ordreAffichage !== null && initialData?.ordreAffichage !== undefined
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await checkSpecialiteCodeAvailable(value, mode === "edit" ? initialData?.id : null);
  ...
  useEffect(() => {
    ...
    if (!value) { setLibelleAvailable(null); return; }
    if (mode === "edit" && value === (initialData?.libelle || "").trim()) { setLibelleAvailable(true); return; }
    const timer = setTimeout(async () => {
      try {
        setCheckingLibelle(true);
        const res = await checkSpecialiteLibelleAvailable(value, mode === "edit" ? initialData?.id : null);
        const available = typeof res === "boolean" ? res : typeof res?.available === "boolean" ? res.available : typeof res?.exists === "boolean" ? !res.exists : null;
  ...
  const handleChange = (field, value) => {
    ...
    if (field === "libelle") setLibelleAvailable(null);
  };
  ...
  const validate = () => {
    ...
    if (libelleAvailable === false) nextErrors.libelle = "Ce libellé existe déjà.";
      ...
      <div className="w-full max-w-xl rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 transition-colors duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">
              {mode === "create" ? "Ajouter une spécialité" : "Modifier la spécialité"}
            </h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Renseignez les informations principales de la spécialité.
            </p>
          </div>
          <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 text-xl transition-colors">
            ×
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.code} onChange={(e) => handleChange("code", e.target.value)} className={inputCls} placeholder="Ex: CARDIO" />
          ...
          <div>
            ...
            <input type="text" value={form.libelle} onChange={(e) => handleChange("libelle", e.target.value)} className={inputCls} placeholder="Ex: Cardiologie" />
            ...
            {!checkingLibelle && libelleAvailable === false  && <p className="mt-1 text-xs text-red-600 dark:text-red-400">Ce libellé existe déjà.</p>}
